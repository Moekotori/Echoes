...
function skip(message) {
  ...
  console.warn(
    '[build-airplay-raop] Skipping optional AirPlay/RAOP native build. Set ECHO_REQUIRE_AIRPLAY_RAOP=1 to make this step fail instead.'
  )
  process.exit(0)
}
...
if (!existsSync(PKG)) {
  const message =
    'Missing optional node_modules/@lox-audioserver/node-libraop. AirPlay/RAOP support will not be included in this build.'
  if (REQUIRE_RAOP_BUILD) {
    fail(`${message} Run npm install first, or install the RAOP native build prerequisites.`)
  }
  skip(message)
}
