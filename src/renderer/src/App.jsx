...
import TrackArtwork, { normalizeArtworkSource } from './components/TrackArtwork'
...
import { preloadLyricsForTrack } from './utils/lyricsPreload'
import {
  clearMediaSession,
...
const ALBUM_GRID_OVERSCAN_ROWS = 4
...
export default function App() {
  ...
  const [albumCoverManualLoadBusy, setAlbumCoverManualLoadBusy] = useState(false)
  ...
  const nextLyricsPreloadKeyRef = useRef('')
  ...
  const fetchCloudTrackCover = async (title, artist, album) => {
    const cleanTitle = String(title || '').trim()
    const cleanArtist = String(artist || '').trim()
    const cleanAlbum = String(album || '').trim()
    ...
    if (window.api?.neteaseSearch) {
      try {
        const songs = await window.api.neteaseSearch(`${cleanTitle} ${cleanArtist}`.trim())
        const bestSong = pickBestCoverCandidate(songs, cleanTitle, cleanArtist, cleanAlbum)
        const cover = normalizeNeteaseCoverUrl(bestSong?.cover)
        if (cover) return cover
      } catch (e) {
        console.warn('Netease track cover prefetch error:', e)
      }
    }
    ...
    try {
      ...
      const data = await response.json()
      const artwork = data?.results?.[0]?.artworkUrl100
      return normalizeItunesCoverUrl(artwork) || null
    } catch (e) {
      console.warn('iTunes track cover prefetch error:', e)
      return null
    }
  }
  ...
  useEffect(() => {
    if (!isPlaying || !currentTrackPath || !nextTrack?.path) return
    if (nextTrack.path === currentTrackPath) return
    if (isRemoteTrackPath(nextTrack.path) && !isStreamingTrackPath(nextTrack.path)) return

    const storedMeta = trackMetaMapRef.current?.[nextTrack.path] || null
    const parsedInfo = parseTrackInfo(nextTrack, storedMeta)
    const title =
      parsedInfo?.title ||
    ...
    const preloadKey = [
      ...
      title,
      artist,
      config.lyricsSource || DEFAULT_CONFIG.lyricsSource
    ].join('::')
    if (nextLyricsPreloadKeyRef.current === preloadKey) return
    nextLyricsPreloadKeyRef.current = preloadKey

    void preloadLyricsForTrack({
      track: nextTrack,
      title,
      artist,
      album,
      ...
      api: window.api,
      requestLrcLib,
      defaultSource: config.lyricsSource || DEFAULT_CONFIG.lyricsSource,
      isStreamingTrack: isStreamingTrackPath(nextTrack.path)
    }).catch((e) => {
      console.warn('[lyrics] next track preload failed', e)
    })
  }, [config.lyricsSource, currentTrackPath, isPlaying, nextTrack, requestLrcLib])
  ...
  const artistNamesSet = useMemo(() => {
    ...
    const s = new Set()
    for (const b of artistBuckets) s.add(normalizeArtistNameKey(b.name))
    return s
  ...
  useEffect(() => {
    ...
    if (!artistNamesSet.has(normalizeArtistNameKey(selectedArtist))) setSelectedArtist('all')
  ...
  const sidebarPlaybackContext = useMemo(() => {
    ...
    if (listMode === 'album' && selectedAlbum && selectedAlbum !== 'all') {
      if (selectedAlbumOrderedTracks.length > 0) {
        const sortedPaths = selectedAlbumOrderedTracks.map((t) => t.path)
  ...
  const albumArtistDetailOpen =
    libraryBrowserVisible &&
    ((listMode === 'album' && selectedAlbum !== 'all') ||
      (listMode === 'artists' && selectedArtist !== 'all'))
  const libraryDetailPrefetchAllowed = !albumArtistDetailOpen || libraryDetailPrefetchReady

  useEffect(() => {
    if (!albumArtistDetailOpen) {
      if (libraryDetailPrefetchTimerRef.current) {
        window.clearTimeout(libraryDetailPrefetchTimerRef.current)
        libraryDetailPrefetchTimerRef.current = null
      }
      setLibraryDetailPrefetchReady((ready) => (ready ? ready : true))
      return undefined
    }

    if (libraryDetailPrefetchReady) return undefined

    const timer = window.setTimeout(() => {
      startTransition(() => {
        setLibraryDetailPrefetchReady(true)
      })
      if (libraryDetailPrefetchTimerRef.current === timer) {
        libraryDetailPrefetchTimerRef.current = null
      }
    }, LIBRARY_DETAIL_PREFETCH_DELAY_MS)
    libraryDetailPrefetchTimerRef.current = timer
  ...
  }, [albumArtistDetailOpen, libraryDetailPrefetchReady, selectedAlbum, selectedArtist])
  ...
  const metadataPrefetchSidebarTracks = useMemo(() => {
    ...
    if (!libraryDetailPrefetchAllowed) return []
  ...
  const metadataContextWarmupSidebarTracks = useMemo(() => {
    ...
    return warmup
  }, [libraryBrowserVisible, libraryDetailPrefetchAllowed, listMode, tracksForSidebarListFiltered])
  ...
  const albumCoverManualLoadTargets = useMemo(() => {
    ...
    return buildAlbumWallHydrateTargets(
      albumGroupsFiltered.flatMap((album) => album.tracks || []),
      trackMetaMap,
      albumCoverMap,
      {
        maxTracksPerAlbum: 3,
        maxAlbums: ALBUM_COVER_MANUAL_LOAD_LIMIT,
  ...
  const handleLoadMissingAlbumCovers = useCallback(() => {
    ...
    setAlbumCoverManualLoadRequest((prev) => ({
      id: prev.id + 1,
    ...
    const run = async () => {
      ...
      try {
        ...
        const runWorker = async () => {
          while (!cancelled && index < pendingTargets.length) {
        ...
        if (!cancelled) {
          setAlbumCoverManualLoadBusy(false)
        }
      }
    }
  ...
  const metadataPrefetchAlbumGroups = useMemo(() => {
    ...
    const columnCount = Math.max(1, visibleAlbumRange.columnCount)
  ...
  }, [albumGroupsFiltered, libraryBrowserVisible, listMode, selectedAlbum, visibleAlbumRange])
  ...
  const metadataPrefetchPlan = useMemo(() => {
    ...
    return buildTrackMetadataPrefetchPlan({
      ...
      visibleSidebarTracks:
        showTrackList && libraryDetailPrefetchAllowed ? visibleSidebarTracks : [],
      metadataPrefetchSidebarTracks:
        showTrackList && libraryDetailPrefetchAllowed ? metadataPrefetchCandidateTracks : [],
  ...
  const visibleCoverHydrationPlan = useMemo(() => {
    return buildVisibleCoverHydrationPlan({
      visibleTracks: showTrackList && libraryDetailPrefetchAllowed ? visibleSidebarTracks : [],
      aheadTracks:
        showTrackList && libraryDetailPrefetchAllowed ? coverPrefetchAheadSidebarTracks : [],
  ...
  useEffect(() => {
    ...
    const runNext = async () => {
      while (!cancelled) {
        ...
        try {
          let cover = await fetchCloudAlbumCover(candidate.albumName, candidate.artist)
          if (!cover) {
            const currentEntry = trackMetaMapRef.current?.[candidate.track.path] || {}
            const info = parseTrackInfo(candidate.track, currentEntry)
            cover = await fetchCloudTrackCover(
              info?.title || candidate.track?.info?.title || '',
              info?.artist || candidate.artist || '',
              candidate.albumName
            )
          }
  ...
  const handlePickAlbumFromSidebar = useCallback(
    (album) => {
      if (albumDetailLeaveTimerRef.current) {
        window.clearTimeout(albumDetailLeaveTimerRef.current)
      ...
      selectedAlbumTracksRef.current = {
        key: albumKey,
        name: album.name,
        paths: new Set(albumTracks.map((track) => track?.path).filter(Boolean)),
        tracks: albumTracks,
  ...
  const handleLocateTrackAlbum = useCallback(
    (track) => {
      ...
      const albumTracks = queryFilteredPlaylist.filter(
  ...
  const handleBackToAlbumOverview = useCallback(() => {
    ...
    albumDetailLeaveTimerRef.current = window.setTimeout(() => {
      ...
      setAlbumDetailLeaving(false)
      albumDetailLeaveTimerRef.current = null
    }, ALBUM_DETAIL_RETURN_ANIMATION_MS)
  }, [])
                        ...
                        {visibleSidebarTracks.map((track) => {
                          ...
                          return (
                            <div
                              ...
                              draggable
                              onDragStart={(e) => {
                                if (isSelectableTextTarget(e.target)) {
                                  e.preventDefault()
                                  return
                                }
                                const dragPaths =
                            ...
                            >
                              ...
                              <div className="track-text-group">
                                ...
                                <div
                                  ...
                                  title={`${displayArtist} - ${trackAlbumName}`}
                                >
                                  <ArtistLink
