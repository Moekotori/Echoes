...
test('visible-row metadata hydrate calls getExtendedMetadata with light-mode options', () => {
  ...
  assert.match(appSource, /buildAlbumThumbnailMetadataRequestOptions\(\)/)
  assert.match(appSource, /getExtendedMetadataHandler\(track\.path,\s*metadataOptions\)/)
  assert.match(
    preloadSource,
...
test('main metadata light mode gates heavy probes and extras', () => {
  assert.match(
    mainSource,
    /options\?\.mode === 'visible-row' \|\| options\?\.mode === 'album-wall'[\s\S]*const isLightweightMode = mode === 'visible-row' \|\| mode === 'album-wall'[\s\S]*includeTechnicalProbe:\s*isLightweightMode\s*\?\s*false/
  )
  assert.match(mainSource, /coverSize:\s*options\?\.coverSize === 'album-thumbnail'/)
  assert.match(mainSource, /shouldRunTechnicalProbe[\s\S]*getFfmpegAudioInfo\(filePath\)/)
  assert.match(
    mainSource,
