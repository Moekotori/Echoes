...
export default function StreamingView({ onPlayTrack }) {
  ...
  const loadProviderPlaylist = useCallback(async (providerId) => {
    const playlistInput = window.prompt(
      t(
        PLAYLIST_PROVIDERS.find((provider) => provider.id === providerId)?.promptKey ||
          'streaming.playlists.prompt',
        'Paste the playlist link or ID'
      )
    )
        ...
        {PLAYLIST_PROVIDERS.map((provider) => {
          ...
          return (
            <button
              ...
              type="button"
              className={`streaming-provider-chip streaming-provider-chip--playlist${loadingPlaylist ? ' active' : ''}`}
              onClick={() => loadProviderPlaylist(provider.id)}
